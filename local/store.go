// Package api provide a storage backend based on the local file system.
//
...
var defaultPathPerm = os.FileMode(0755)
...
func (store FileStore) NewUpload(info tusd.FileInfo) (id string, err error) {
	...
	err = os.MkdirAll(filepath.Dir(store.binPath(id)), defaultPathPerm)
	if err != nil {
		return "", err
	}
